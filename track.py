    ...
    for frame_idx, (path, im, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                ...
                start_point = (im0.shape[1]-350, 0)
                end_point = (im0.shape[1]-350, im0.shape[0])
                cv2.line(im0, start_point, end_point, color, thickness=2)


                start_point = (im0.shape[1]-300, 0)
                end_point = (im0.shape[1]-300, im0.shape[0])
                cv2.line(im0, start_point, end_point, color, thickness=2)
                thickness = 2
                ...
                start_point = (0, im0.shape[0]-50)
                end_point = (im0.shape[1], im0.shape[0]-50)
                cv2.line(im0, start_point, end_point, color, thickness=2)
...
def count_obj(box,w,h,id):
    global count, customer, data, person
     
    center_coordinates = (int(box[0]+(box[2]-box[0])/2) , int(box[1]+(box[3]-box[1])/2))
    #center_coordinates = (int(box(0), int(box(1))))
    
    #for vertical line I used x coordinate of centre
    if int(box[0]+(box[2]-box[0])/2) >= (w-350) and int(box[0]+(box[2]-box[0])/2) <= (w-300) :
        if id not in data:
            count += 1
            data.append(id)
    
    #for horizontal line I used y coordinate of centre
    """ elif int(box[1]+(box[3]-box[1])/2) > (h-50): #and int(box[1]+(box[3]-box[1])/2) < (h-20):
        if id not in data:
            customer += 1
            data.append(id) """
